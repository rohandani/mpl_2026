-- ============================================================
-- MPL 2026 — Add earliest-submission tiebreaker to match leaderboard
-- ============================================================
-- When users tie on total_points, the user who submitted their
-- prediction first (earliest created_at) ranks higher.
-- Also returns submitted_at for display on the leaderboard.
-- ============================================================

DROP FUNCTION IF EXISTS get_match_leaderboard(UUID);
...
AS $$
  ...
  WHERE mp.fixture_id = p_fixture_id
    ...
    AND ms.id = 'default'
  ORDER BY total_points DESC, mp.created_at ASC;
$$;
